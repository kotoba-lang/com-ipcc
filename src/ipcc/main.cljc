(ns ipcc.main
  "Kotodama WASM entrypoint for the Ipcc clean-room actor (L4) — Clojure port.
  ...
  facts (`ipcc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Application" :plural "applications" :id-prefix "ipcc_app"
    :fields [:applicantId :type :number :status]      :required [:type :number]
    :coerce {}                       :refs {}}
   {:entity "Grant"         :plural "grants"         :id-prefix "ipcc_gra"
    :fields [:applicationId :number :grantedAt]      :required [:number :grantedAt]
    :coerce {}                       :refs {:applicationId "Application"}}
   {:entity "Surveillance"  :plural "surveillances"  :id-prefix "ipcc_sur"
    :fields [:disease :region :cases :reportedAt]    :required [:disease :region]
    :coerce {:cases :int}            :refs {}}
   {:entity "Measurement"   :plural "measurements"   :id-prefix "ipcc_mea"
    :fields [:station :parameter :value :measuredAt] :required [:station :parameter]
    :coerce {:value :float}          :refs {}}
   {:entity "Patent"        :plural "patents"        :id-prefix "ipcc_pat"
    :fields [:applicantId :title :number :status]    :required [:title :number]
    :coerce {}                       :refs {}}
   {:entity "Trial"         :plural "trials"         :id-prefix "ipcc_tri"
    :fields [:sponsor :phase :status]                :required [:sponsor :phase]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ipcc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ipcc-compat" :tier tier :entities entities} 200])
